...
public class PickupItem : MonoBehaviour, IWeaponAction
{
    ...
    protected RunUpgrades Upgrades { get; private set; }
    GameObject replacementModel;
    public void SetUpgradeModel(GameObject prefab)
    {
        if (prefab == null || prefab == replacementModel || Visual == null) return;
        replacementModel = prefab;
        foreach (Transform child in Visual) { child.gameObject.SetActive(false); Destroy(child.gameObject); }
        var model = Instantiate(prefab, Visual);
    ...
    public void Equip(Camera camera, Transform player, Transform socket = null)
    {
        OwnerCamera = camera;
        Owner = player;
        Upgrades = player.GetComponent<RunUpgrades>();
        IsHeld = true;
