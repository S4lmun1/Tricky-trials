...
public class WeaponManager : MonoBehaviour
{
    ...
    public GameObject EquippedObject => currentWeaponIndex >= 0 && currentWeaponIndex < weapons.Count
        ? weapons[currentWeaponIndex].weaponObject : null;
    public ItemData EquippedItem
    {
        get
        {
            var equipped = EquippedObject;
            if (equipped == null) return null;
            // Read the actual held component, even when a slot's cached definition is stale.
            if (equipped.TryGetComponent<PickupItem>(out var pickup) && pickup.itemData != null) return pickup.itemData;
            if (equipped.TryGetComponent<Gun>(out var gun) && gun.itemData != null) return gun.itemData;
            return weapons[currentWeaponIndex].definition;
        }
    }
